...
extension DatabaseManager {
    ...
    /// Creates a new conversation with target user emial and first message sent
    public func createNewConversation(with otherUserEmail: String, name: String, firstMessage: Message, completion: @escaping (Bool) -> Void) {
        ...
        ref.observeSingleEvent(of: .value, with: { [weak self] snapshot in
            ...
            if var conversations = userNode["conversations"] as? [[String: Any]] {
                // Conversation array exists for current user, you shoud append
                ...
                // Conversation dose NOT exist, create it
    ...
    public func getAllConversations(for email: String, completion: @escaping (Result<[Conversation], Error>) -> Void) {
        database.child("\(email)/conversations").observe(.value, with: { snapshot in
            ...
            let conversations: [Conversation] = value.compactMap({ dictionary in
                ...
                let latestMessageObject = LatestMessage(text: message,
                                                        date: date,
                                                        isRead: isRead)
                    ...
                    strongSelf.database.child("\(currentEmail)/conversations").setValue(databaseEntryConversations, withCompletionBlock: { error, _ in
                        ...
                        strongSelf.database.child("\(otherUserEmail)/conversations").observeSingleEvent(of: .value, with: { snapshot in
                            ...
                            if var otherUserConversations = snapshot.value as? [[String: Any]] {
                                ...
                                // Current collection dose not exist
                                let newConversationData: [String: Any] = [
                                    "id": conversationId,
                                    "other_user_email": DatabaseManager.safeEmail(emailAddress: otherUserEmail),
                                    "name": currentName,
...
struct ChatAppUser {
    ...
    var profilePictureFileName: String {
        // nkkha-ityu-gmail-com_profile_picture.png
